...
public class ImportConfig {
    ...
    public IntegrationFlow fileMoveFlow() {
        return IntegrationFlows.from(sourceDirectory(), conf -> conf.poller(Pollers.fixedDelay(2000)))
                ...
                .filter(msg -> !((String) msg).isBlank())
                .transform(this::csvStringToProductConverter)
                .handle(jpaPersistHandler(), ConsumerEndpointSpec::transactional)
                .get();
    }

    private Product csvStringToProductConverter(String csvString) {
        if (!csvString.isBlank()) {
            String[] params = csvString.split(",");
            return new Product(params[0], BigDecimal.valueOf(Double.parseDouble(params[1])));
        }
        return null;
    }
